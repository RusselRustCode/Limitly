"""
Simulator Router — запуск синтетической симуляции через API.

Эндпоинты:
  POST /simulator/run          — запустить симуляцию и сохранить логи в БД
  POST /simulator/run-and-analyze — симуляция + немедленный анализ
  GET  /simulator/presets      — список доступных пресетов когорт
  POST /simulator/evolution    — N итераций эволюции контента
"""
...
async def run_simulation(
    ...
    """
    Генерирует синтетические логи студентов и сохраняет их в БД.

    Использует IRT 3PL модель для симуляции ответов.
    Полезно для тестирования модуля анализа без реальных студентов.
    """
    _validate_cohort_type(req.cohort_type)
    ...
    saved = await repo.save_logs_bulk(logs)
...
@simulator_router.post("/run-and-analyze", summary="Симуляция + анализ")
async def run_and_analyze(
    ...
    """
    Полный цикл: генерирует логи → сохраняет → запускает анализ → возвращает директивы.

    Идеально для демонстрации работы системы.
    """
    _validate_cohort_type(req.cohort_type)
    ...
    # Принудительный анализ (force=True, не ждём порога N)
...
def _validate_cohort_type(cohort_type: str):
